app.controller('DetailVideoController', function($location, $routeParams, videoListValue) {
    var video = videoListValue.filter(function(v){
        return v.id == $routeParams.videoId
    })
    if(video.length == 0){
        $location.path('/')
    }else{
    ...
    function embedChat() {
        d3.csv(video.path.chatlog, function(d, i, columns){
            return d;
        }, function (error, data) {
            if (error) throw error;
            var chatDicts = {};
